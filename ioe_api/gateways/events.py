...
from iot.iot.doctype.iot_device_event.iot_device_event import count_device_event, query_device_event, get_event_detail
...
def list(name, start=0, limit=40, filters=None):
	try:
		...
		frappe.response.update({
			"ok": True,
			"data": query_device_event(sn=name, start=start, limit=limit, filters=filters)
		})
...
def count(name, filters=None):
	try:
		...
		frappe.response.update({
			"ok": True,
			"data": count_device_event(sn=name, filters=filters)
		})
	except Exception as ex:
		frappe.response.update({
			"ok": False,
