...
def list(name, start=0, limit=40, filters=None):
	try:
		...
		frappe.response.update({
			"ok": True,
			"activities": query_device_event_by_user(user=name, start=start, limit=limit, filters=filters)
		})
...
def count(name, filters=None):
	try:
		...
		frappe.response.update({
			"ok": True,
			"activities": count_device_event_by_user(user=name, filters=filters)
		})
	except Exception as ex:
		frappe.response.update({
			"ok": False,
