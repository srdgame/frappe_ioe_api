...
from iot.iot.doctype.iot_device_activity.iot_device_activity import query_logs_by_user, count_logs_by_user, get_log_detail
...
def list(name, start=0, limit=40, filters=None):
	try:
		...
		frappe.response.update({
			"ok": True,
			"activities": query_logs_by_user(user=name, start=start, limit=limit, filters=filters)
		})
...
def count(name, filters=None):
	try:
		...
		frappe.response.update({
			"ok": True,
			"activities": count_logs_by_user(user=name, filters=filters)
		})
	except Exception as ex:
		frappe.response.update({
			"ok": False,
